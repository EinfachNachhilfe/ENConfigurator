...
birthdayStudent.addEventListener('input', function() {
  if (!isValidDate(birthdayStudent.value)) {
    // Mark input as invalid
    birthdayStudent.classList.add('invalid');
  } else {
    // Mark input as valid
    birthdayStudent.classList.remove('invalid');
  }
});
...
function isValidDate(dateString) {
  ...
  // Extract day, month, and year values from string
  var parts = dateString.split('.');
  var day = parseInt(parts[0], 10);
  var month = parseInt(parts[1], 10);
  var year = parseInt(parts[2], 10);

  // Create new Date object with extracted values
  var date = new Date(year, month - 1, day);

  // Check if the new Date object has a different day, month, or year value from the extracted values
