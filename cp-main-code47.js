...
const applyValidation = (inputElement, emptyErrorMsg, invalidErrorMsg, pattern = null) => {
  ...
  if (validationImageWrapper) {
   ...
   const handleValidation = () => {
    const radioButtonGroup = inputElement.parentNode.parentNode.parentNode; // Get parent of radio button group
    const isSelected = [...radioButtonGroup.querySelectorAll('input[type="radio"]')].some(radio => radio.checked);

    if (inputElement.hasAttribute('required') && !isSelected) {
...
// Usage example
const radioButtons = document.querySelectorAll('input[type="radio"][name="gender_tutor"]');
radioButtons.forEach(radioButton => applyValidation(radioButton, 'Please select a shipping option', 'Invalid shipping option'));
